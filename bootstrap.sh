...
# instal instant client
apt-get -y install libaio1 alien
cp /vagrant/oracle/*.rpm /tmp/
cd /tmp
...
### PATH ###########################################################
if ! echo $PATH | grep -q "$ORACLE_HOME/bin:$ORACLE_HOME/ctx/lib"
then
    PATH=$PATH:$ORACLE_HOME/bin
    PATH=$PATH:$ORACLE_HOME/ctx/lib
fi
...
echo '. /etc/profile.d/oracle.sh' >> /usr/local/apache2/bin/envvars
